...
async function updateInventoryOnTransfer(assetId, quantity, fromBaseId, toBaseId) {
  const assetObjId = mongoose.Types.ObjectId(assetId);
  const fromBaseObjId = mongoose.Types.ObjectId(fromBaseId);
  const toBaseObjId = mongoose.Types.ObjectId(toBaseId);
